// see 3-2-2026 for no long presses.  This one has long presses.
...
// Button pins - note these are integers
#define BTN_DOWN   A0
#define BTN_UP     A1
#define BTN_BACK   A2
...
// ===== MENU STRUCTURE =====
// strucutres must be listed first to compile in Arduino
struct MenuItem { 
  const char* name;
...
void handleButtonPress() {
    ...
    int pressedButton = -1;
    if (isButtonPressed(BTN_UP)) pressedButton = BTN_UP;
    else if (isButtonPressed(BTN_DOWN)) pressedButton = BTN_DOWN;
    else if (isButtonPressed(BTN_SELECT)) pressedButton = BTN_SELECT;
    else if (isButtonPressed(BTN_BACK)) pressedButton = BTN_BACK;
    ...
    if (pressedButton != -1) {
        ...
        if (!longPressTriggered && (now - buttonPressStart >= longPressDelay)) {
            switch (currentButton) {
                case BTN_UP:     navigateDown(); break;
                case BTN_DOWN:   navigateUp(); break;
                case BTN_SELECT: goBack(); break;
                case BTN_BACK:   selectItem(); break;
            }
            longPressTriggered = true; // Prevent short press later
        }
    }
    // If button released
    else if (currentButton != -1) {
        if (!longPressTriggered) {
            // Short press action only if long press wasn't triggered
            switch (currentButton) {
                case BTN_UP:     navigateUp(); break;
                case BTN_DOWN:   navigateDown(); break;
                case BTN_SELECT: selectItem(); break;
                case BTN_BACK:   goBack(); break;
            }
        }
        // Reset state
        currentButton = -1;
